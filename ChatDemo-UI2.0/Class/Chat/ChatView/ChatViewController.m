...
#define eConversationTypeChatRoom EMConversationTypeChatRoom
#define eConversationTypeGroupChat EMConversationTypeGroupChat
#define eConversationTypeChat EMConversationTypeChat
...
//            if (error && (error.errorCode != EMErrorChatroomJoined))
            if (error && error.errorCode != EMErrorChatroomAlreadyJoined)
            {
...
//    [self loadMoreMessagesFrom:timestamp count:KPageCount append:NO];
    ...
    if ([self.messages count] > 0) {
        messageId = [(EMMessage *)self.messages.firstObject messageId];
    }
    else {
        messageId = nil;
    }
    [self _loadMoreMessagesBefore:messageId count:10 append:YES];
...
- (void)slimeRefreshStartRefresh:(SRRefreshView *)refreshView
{
    ...
    if (firstMessage)
    {
        [self _loadMoreMessagesBefore:firstMessage.messageId count:KPageCount append:YES];
    }
...
- (void)chatVideoCellPressed:(MessageModel *)model
{
    ...
    {
        NSString *localPath = model.message == nil ? model.localPath : ((EMFileMessageBody *)model.message.body).localPath;
    ...
    [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
        ...
        if (!error) {
            ...
            NSString *localPath = aMessage == nil ? model.localPath :((EMFileMessageBody *)model.message.body).localPath;
            if (localPath && localPath.length > 0) {
...
-(void)chatImageCellBubblePressed:(MessageModel *)model
{
    __weak ChatViewController *weakSelf = self;
    id <IChatManager> chatManager = [[EaseMob sharedInstance] chatManager];
    if (model.message.body.type == eMessageBodyType_Image) {
        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                ...
                if (!error) {
                    ...
                    NSString *localPath = aMessage == nil ? model.localPath : ((EMImageMessageBody *)model.message.body).localPath;
                    if (localPath && localPath.length > 0) {
                        UIImage *image = [UIImage imageWithContentsOfFile:localPath];
    ...
    }else if ([model.message.body messageBodyType] == eMessageBodyType_Video) {
        //获取缩略图
        EMVideoMessageBody *videoBody = (EMVideoMessageBody *)model.message.body;
...
- (void)didReceiveOfflineMessages:(NSArray *)offlineMessages
{
    ...
    _chatTagDate = nil;
//    long long timestamp = [[NSDate date] timeIntervalSince1970] * 1000 + 1;
    [self _loadMoreMessagesBefore:nil count:KPageCount append:YES];
...
//            voice.duration = aDuration;
            EMVoiceMessageBody *voice = [[EMVoiceMessageBody alloc] initWithLocalPath:recordPath displayName:@"audio"];
            voice.duration = (int)aDuration;
...
//        EMChatVideo *chatVideo = [[EMChatVideo alloc] initWithFile:[mp4 relativePath] displayName:@"video.mp4"];
        EMVideoMessageBody *chatVideo = [[EMVideoMessageBody alloc] initWithLocalPath:[mp4 relativePath] displayName:@"video.mp4"];
...
- (void)_loadMoreMessagesBefore:(NSString *)messageId count:(NSInteger)count append:(BOOL)append
{
    __weak typeof(self) weakSelf = self;
    dispatch_async(_messageQueue, ^{
//        NSArray *messages = [weakSelf.conversation loadNumbersOfMessages:count before:timestamp];
#warning 李飞
        NSArray *messages = [weakSelf.conversation loadMoreMessagesFromId:messageId limit:count];
        if ([messages count] > 0) {
...
-(void)sendAudioMessage:(EMVoiceMessageBody *)voice
{
    NSDictionary *ext = nil;
...
-(void)sendVideoMessage:(EMVideoMessageBody *)video
{
    NSDictionary *ext = nil;
